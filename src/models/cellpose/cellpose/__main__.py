...
def main():
    print(f"\n\nUsing Cellpose in: {cellpose.__file__} \n\n")
    ...
    output_args.add_argument('--model_dir', type=str, help='Path to where the models will be stored')
    ...
    training_args.add_argument('--config_train', type = str, default=None, help = 'Path to the training yaml file')
    # misc settings
        ...
        if not args.train and not args.train_size:
            ...
            if args.train:       
                if args.config_train is not None:
                    with open(args.config_train, 'r') as stream:
                        config_train = yaml.safe_load(stream)
                else:
                    config_train = None
                cpmodel_path = model.train(images, labels, train_files=image_names,
                                           ...
                                           save_path=os.path.realpath(args.model_dir), save_every=args.save_every,
                                           save_each=args.save_each,
                                           rescale=rescale,n_epochs=args.n_epochs,
                                           batch_size=args.batch_size, omni=args.omni, config_train = config_train)
                model.pretrained_model = cpmodel_path
                logger.info('>>>> model trained and saved to %s'%cpmodel_path)
                #with open(os.path.join(args.model_dir, 'model_path.yml'), 'w') as yaml_file:
	            #    yaml.dump({'model_path': cpmodel_path},yaml_file, default_flow_style=False)

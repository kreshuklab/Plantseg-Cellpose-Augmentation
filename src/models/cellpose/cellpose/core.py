...
import tempfile
import cv2
from . import transforms, dynamics, utils, plot, metrics, transforms_plantseg
import time 
import skimage.io
...
#             print('>>> Using Ranger21 optimizer')
    ...
    def _train_net(self, train_data, train_labels, 
              ...
              SGD=False, batch_size=8, rescale=True, netstr='cellpose', config_train = None): 
        """ train function uses loss function self.loss_fn in models.py"""

        nan_args = np.nonzero([np.sum(np.isnan(t)) for t in train_data])[0]
        nan_args.sort()
        for index in reversed(nan_args):
        ...
        # Create transforms
        phase_config = config_train['transformer']
        ## Get min max ... values
        minval = np.min([t[0].min() for t in train_data])
        maxval = np.max([t[0].max() for t in train_data])
        meanval = np.mean([t[0].mean() for t in train_data])
        stdval = np.mean([t[0].std() for t in train_data])
        t = transforms_plantseg.get_transformer(phase_config,minval, maxval, meanval, stdval)
        self.raw_transform = t.raw_transform()
        self.label_transform = t.label_transform()
        ...
        for iepoch in range(self.n_epochs):
            ...
            #for i,(td, tl) in enumerate(zip(train_data, train_labels)):
            #    train_data[i] = self.raw_transform(td)
            #    train_labels[i] = self.label_transform(tl)
            #print("Transforms took: ", time.time()-start, " sec")

            #t_train_data = train_data
            #t_train_labels = train_labels
            for ibatch in range(0,nimg,batch_size):
                ...
                # PUT TRANSFORM HERE
                # train_data - 
                #   - list of images 2x400x400
                #   - first channel 0 -  ~1 (percentile thing)
                #   - secound channel 0 - 0
                #   - dtype float32
                # train_labels -
                #   - list of flows 4x400x400
                #   - first channel - labels (0 - 200)
                #   - secound channel - cell prob 0 - 1
                #   - third / fourth channel - flow -1 - 1
                # ################
                # Below, take away rotation....
                #
                scale_range = 0.
                do_flip = False
                gamma_range = 0
                imgi, lbl, scale = transforms.random_rotate_and_resize(
                                        ...
                                        omni=self.omni, do_flip=do_flip, gamma_range=gamma_range)

...
class Scale:
    ...
    def __init__(self, random_state, scale_range, order = 0, execution_probability = 0.1, **kwargs):
    ...
    def __call__(self, m):
        if self.random_state.uniform() > self.execution_probability: return m
        for batch in range(len(m)):
            ...
            if m.shape[1] == 3:
                # for cellpose labels
                for layer in range(m.shape[1]): 
                    order = 0 if layer == 0 else 3
                    cut_shape = tuple(np.ceil(d/scale).astype('int') for d in m[batch][layer].shape)
                    center_slice = tuple(slice((d2-d1)//2,(d2-d1)//2+d1) for d1,d2 in zip(cut_shape, m[batch][layer].shape))  
                    cut_m = m[batch][layer][center_slice]
                    scaled_m = scipy.ndimage.zoom(cut_m, scale, order = order)
                    slize = tuple(slice(0,d) for d in m[batch][layer].shape)
                    m[batch][layer] = scaled_m[slize]
            elif m.shape[1]== 2:
                # for cellpose raw
                m_ = m[batch][0]
                cut_shape = tuple(np.ceil(d/scale).astype('int') for d in m_.shape)
                center_slice = tuple(slice((d2-d1)//2,(d2-d1)//2+d1) for d1,d2 in zip(cut_shape, m_.shape))  
                cut_m = m_[center_slice]
                m[batch][0] = TF.resize(cut_m.unsqueeze(0), m_.shape)
        return m

class MotionBlur:
    """
    Blur the image in a random direction
    """

    def __init__(self, random_state, blur_range=(1, 8), execution_probability=0.1, **kwargs):
        self.random_state = random_state
        assert len(blur_range) == 2
    ...
    def __call__(self, m):
        ...
        # Draw random
        angle = self.random_state.uniform(-90, 90)
        kernel_radius = self.random_state.randint(self.blur_range[0], self.blur_range[1])
        # define constants
        kernel_diameter = 2*kernel_radius + 1
        kernel = torch.ones(*((1,)*(m.ndim-1)),kernel_diameter, device = m.device)/kernel_diameter
        m_shape = m.shape
        m_ = m[:,0:1,:,:]

        # transform
        m_ = F.pad(m_, (kernel_radius,)*4, mode='reflect')
        m_ = TF.rotate(m_, angle=angle ,expand = True, interpolation = self.interpolation)
        m_ = F.conv2d(m_, kernel,padding='same')
        m_ = TF.rotate(m_, angle=-angle ,expand = False, interpolation = self.interpolation)
        m_ = TF.center_crop(m_, m_shape[-2:])
        m[:,0,:,:] = m_.squeeze()
        return m
...
class RandomFlip:
    ...
    def __init__(self, random_state,axis_prob=0.5, execution_probability = 1.0, **kwargs):
        assert random_state is not None, 'RandomState cannot be None'
        self.random_state = random_state
    ...
    def __call__(self, m):
        if self.random_state.uniform() > self.execution_probability: return m
        # if first axis is 4 long, assume flow, otherwise 
        # If m.shape[0] == 2, assume we have raw flow
        
        for batch in range(len(m)):
            flip = (self.random_state.uniform() < self.axis_prob, self.random_state.uniform() < self.axis_prob)
            flip_axis = tuple(i+1 for i,f in enumerate(flip) if f)
            if  m.shape[1] == 2:
                m[batch] = torch.flip(m[batch], dims = flip_axis)
            elif m.shape[1] == 3:
                m[batch] = np.flip(m[batch], axis = flip_axis)
                if flip[0]:
                    m[batch][1] *= -1 
                if flip[1]:
                    m[batch][2] *= -1 
            else:
                assert False, "Should never get her in RandomFlip"
        return m
...
class RandomRotate90:
    ...
    def __init__(self, random_state,execution_probability = 1.0, **kwargs):
        self.random_state = random_state
        self.execution_probability = execution_probability
    ...
    def __call__(self, m):
        ...
        for batch in range(len(m)):
            ...
            if  m.shape[1] == 2:
                m[batch] = torch.rot90(m[batch], k, (1,2))
            elif m.shape[1] == 3:
                m[batch] = np.rot90(m[batch], k, (1,2))
                if  k == 0:
                    pass
                elif k == 1:
                    m[batch][2], m[batch][1] = m[batch][1], -m[batch][2]
                elif k == 2:
                    m[batch][1] = -m[batch][1]
                    m[batch][2] = -m[batch][2]
                elif k==3:
                    m[batch][1], m[batch][2] = m[batch][2], -m[batch][1]
            else:
                assert False, "Should never get her in RandomRotate90"
        return m
...
class RandomRotate:
    ...
    def __init__(self, random_state, execution_probability= 0.5, angle_spectrum=30, mode='reflect', order=0, **kwargs):
        self.random_state = random_state
    ...
    def __call__(self, m):
        ...
        for batch in range(len(m)):
            ...
            if  m.shape[1] == 2:
                m[batch] = torch_rotate(m[batch], angle)
            elif m.shape[1] == 3:
                m[batch] = rotate(m[batch], angle, axes=(1,2), reshape=False, order=self.order, mode='constant', cval=0)
                radians = np.radians(angle)
                m1 = m[batch][1].copy()
                m[batch][1] = m1*np.cos(radians) - m[batch][2]*np.sin(radians)
                m[batch][2] = m1*np.sin(radians) + m[batch][2]*np.cos(radians)
            else:
                assert False, "Should never get her in RandomRotate"
        return m
...
class RandomContrast:
    ...
    def __call__(self, m):
        ...
        for batch in range(len(m)):
            ...
            if  m.shape[1] == 2:
                m[batch][0] = self.mean + alpha * (m[batch][0] - self.mean)
        return m
...
class AdditiveGaussianNoise:
    ...
    def __call__(self, m):
        ...
        for batch in range(len(m)):
            ...
            gaussian_noise = self.random_state.normal(0, std, size=m[batch][0].shape)
            m[batch][0] = m[batch][0]  + torch.from_numpy(gaussian_noise).to(m.device)
        return torch.clip(m, 0)
...
class AdditivePoissonNoise:
    ...
    def __call__(self, m):
        ...
        for batch in range(len(m)):
            ...
            poisson_noise = self.random_state.poisson(lam, size=m[batch][0].shape)
            m[batch][0] = m[batch][0]  + torch.from_numpy(poisson_noise).to(m.device)
        return m
...
class Transformer:
    ...
    def _transformer_class(class_name):
        m = importlib.import_module('cellpose.transforms_plantseg')

...
class Scale:
    ...
    def __call__(self, m):
        ...
        if scale == 1.0:
            ...
            cut_shape = tuple(np.ceil(d/scale).astype('int') for d in m.shape)
            center_slice = tuple(slice((d2-d1)//2,(d2-d1)//2+d1) for d1,d2 in zip(cut_shape, m.shape))  
            cut_m = m[center_slice]
            scaled_m = scipy.ndimage.zoom(cut_m, scale,order = self.order)
            slize = tuple(slice(0,d) for d in m.shape)
            return scaled_m[slize]
        else:
